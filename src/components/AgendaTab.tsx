...
export default function AgendaTab({ citas, vehiculos, clientes, tecnicos, ordenes, onAddCita, onUpdateCita, onDeleteCita, onCreateOT }: Props) {
  ...
  const handleSave = () => {
    ...
    const existente = editingId ? citas.find((c) => c.id === editingId) : undefined;
    const cita: Cita = {
      id: editingId ?? genId('cita'),
    ...
    if (editingId) onUpdateCita(cita);
    else onAddCita(cita);
  ...
  const handleDeleteConfirmed = () => {
    ...
    onDeleteCita(confirmDelete.id);
    setConfirmDelete(null);
  };

  const handleCambiarEstado = (c: Cita, estado: Cita['estado']) => {
    onUpdateCita({ ...c, estado });
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 space-y-4">
        ...
        <button onClick={handleConfirmar}
          className="w-full flex items-center justify-center gap-2 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-lg transition cursor-pointer">
          <Check className="w-4 h-4" /> Crear orden de trabajo
        </button>
      </div>
    </div>
  );
