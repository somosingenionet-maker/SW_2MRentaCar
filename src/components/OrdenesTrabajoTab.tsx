...
export default function OrdenesTrabajoTab({ ordenes, vehiculos, clientes, tecnicos, onAdd, onUpdate, onDelete }: Props) {
  ...
  const cliLabel = (id: string) => {
    if (id === FLOTA_CLIENTE_ID) return 'Flota propia';
                ...
                {filtered.map(ot => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-slate-700">{ot.clienteId === FLOTA_CLIENTE_ID ? 'Flota propia' : cli ? `${cli.nombre} ${cli.apellidos}` : '—'}</td>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
                ...
                <div>
                  ...
                  <select value={editForm.vehiculoId}
                    onChange={e => {
                      ...
                      setEditForm(f => ({ ...f, vehiculoId: vehId, clienteId: veh?.esFlotaAlquiler ? FLOTA_CLIENTE_ID : (dueno?.id ?? '') }));
                    }}
                ...
                <div>
                  ...
                  {(() => {
                    const veh = vehiculos.find(v => v.id === editForm.vehiculoId);
                    const dueno = clientes.find(c => c.vehiculosAsociados?.includes(editForm.vehiculoId));
                    if (veh?.esFlotaAlquiler) return (
                      <div className="mt-1 px-3 py-2 border border-violet-200 bg-violet-50 rounded-lg text-sm text-violet-800 font-medium flex items-center justify-between">
                        <span>Flota propia — {getEmpresaConfig().nombre}</span>
                        <span className="text-[10px] text-violet-400">Vehículo de la empresa</span>
                      </div>
                    );
                    if (dueno) return (
            ...
            >
              ...
              <form onSubmit={handleCreateSubmit} className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={e => {
                      ...
                      const veh = vehiculos.find(v => v.id === vehId);
                      const dueno = clientes.find(c => c.vehiculosAsociados?.includes(vehId));
                      setOTForm(f => ({ ...f, vehiculoId: vehId, clienteId: veh?.esFlotaAlquiler ? FLOTA_CLIENTE_ID : (dueno?.id ?? '') }));
                    }}
                    className="w-full mt-1 px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                    required
                  >
                    ...
                    {vehiculos.map(v => {
                      ...
                      const sufijo = v.esFlotaAlquiler ? ' (Flota propia)' : dueno ? ` (${dueno.nombre} ${dueno.apellidos})` : '';
                      return (
                        <option key={v.id} value={v.id}>
                          {v.marca} {v.modelo} · {v.matricula}{sufijo}
                        </option>
                      );
                    })}
                  </select>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Cliente *</label>
                  {(() => {
                    const veh = vehiculos.find(v => v.id === otForm.vehiculoId);
                    const dueno = clientes.find(c => c.vehiculosAsociados?.includes(otForm.vehiculoId));
                    if (veh?.esFlotaAlquiler) {
                      return (
                        <div className="mt-1 px-3 py-2 border border-violet-200 bg-violet-50 rounded-lg text-sm text-violet-800 font-medium flex items-center justify-between">
                          <span>Flota propia — {getEmpresaConfig().nombre}</span>
                          <span className="text-[10px] text-violet-400 font-normal">Vehículo de la empresa</span>
                        </div>
                      );
                    }
                    if (dueno) {
                  ...
                  <div className="max-w-3xl mx-auto my-8 bg-white shadow-xl rounded-xl print:shadow-none print:rounded-none print:my-0 print:max-w-none">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      <div>
                        ...
                        <p className="font-semibold text-slate-800">{ot.clienteId === FLOTA_CLIENTE_ID ? `Flota propia — ${empresa.nombre}` : cli ? `${cli.nombre} ${cli.apellidos}` : '—'}</p>
