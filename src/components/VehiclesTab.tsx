...
export default function VehiclesTab({
  ...
  // Actualiza una tarifa de temporada manteniendo las otras dos.
  const updateAddTarifa = (key: keyof TarifasAlquiler, value: string) => {
    const num = value === '' ? 0 : Number(value);
    setFormData(fd => ({
      ...fd,
      tarifasAlquiler: {
        temporadaAlta: fd.tarifasAlquiler?.temporadaAlta ?? 0,
        temporadaMedia: fd.tarifasAlquiler?.temporadaMedia ?? 0,
        temporadaBaja: fd.tarifasAlquiler?.temporadaBaja ?? 0,
        [key]: num,
      },
    }));
  };
  const updateEditTarifa = (key: keyof TarifasAlquiler, value: string) => {
    const num = value === '' ? 0 : Number(value);
    setEditFormData(ef => ef ? {
      ...ef,
      tarifasAlquiler: {
        temporadaAlta: ef.tarifasAlquiler?.temporadaAlta ?? 0,
        temporadaMedia: ef.tarifasAlquiler?.temporadaMedia ?? 0,
        temporadaBaja: ef.tarifasAlquiler?.temporadaBaja ?? 0,
        [key]: num,
      },
    } : ef);
  };
  ...
  const handleExportCsv = () => {
    ...
    downloadCsv(`${slugify(getEmpresaConfig().nombre)}_flota_${new Date().toISOString().slice(0, 10)}.csv`, [headers, ...rows]);
  };
                      ...
                      <td className="py-3.5 px-2">
                        <div>
                          <div className="flex items-center gap-1.5">
                            ...
                            {veh.esFlotaAlquiler && hasAlquileres && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-blue-100 text-blue-700">Alquiler</span>}
                          </div>
        ...
        <div className="lg:col-span-5">
          {selectedVehiculo ? (
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-6" id={`detail-panel-${selectedVehiculo.id}`}>
              <div className="flex justify-between items-start border-b border-slate-50 pb-4">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {selectedVehiculo.esFlotaAlquiler && hasAlquileres && (
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">Alquiler</span>
                    )}
                  </div>
              ...
              {/* Tarifas de alquiler por temporada */}
              {selectedVehiculo.esFlotaAlquiler && hasAlquileres && (
                <div className="bg-blue-50/50 rounded-xl border border-blue-100 p-4">
                  <div className="text-[10px] text-blue-500 font-semibold uppercase flex items-center gap-1 mb-2">
                    <Calendar className="w-3 h-3 text-blue-500" /> Tarifas de alquiler (€/día)
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    {([
                      { key: 'temporadaAlta' as const, label: 'Alta' },
                      { key: 'temporadaMedia' as const, label: 'Media' },
                      { key: 'temporadaBaja' as const, label: 'Baja' },
                    ]).map(({ key, label }) => {
                      const precio = selectedVehiculo.tarifasAlquiler?.[key];
                      return (
                        <div key={key} className="bg-white rounded-lg border border-blue-100 p-2 text-center">
                          <div className="text-[10px] text-slate-400 font-semibold uppercase">{label}</div>
                          <div className="text-sm font-extrabold text-slate-800 mt-0.5">{precio != null ? `${precio} €` : '—'}</div>
                        </div>
                      );
                    })}
                  </div>
                ...
                {hasAlquileres && (
                  <div className="bg-blue-50 border border-blue-100 rounded-xl p-3">
                    <label className="flex items-center gap-2.5 cursor-pointer select-none">
                      <input
                        type="checkbox"
                        checked={formData.esFlotaAlquiler ?? false}
                        onChange={e => setFormData({ ...formData, esFlotaAlquiler: e.target.checked })}
                        className="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                      />
                      <span className="text-xs font-bold text-blue-800">Vehículo de alquiler</span>
                    </label>
                    <p className="text-[11px] text-blue-500 mt-1 ml-6">Solo los vehículos marcados aparecerán disponibles en Alquileres y Contratos.</p>

                    {formData.esFlotaAlquiler && (
                      <div className="mt-3 ml-6">
                        <p className="text-[11px] font-bold text-blue-800 mb-1.5">Tarifas por temporada (€/día)</p>
                        <div className="grid grid-cols-3 gap-2">
                          {([
                            { key: 'temporadaAlta' as const, label: 'Alta' },
                            { key: 'temporadaMedia' as const, label: 'Media' },
                            ...
                            <div key={key}>
                              <label className="block text-[10px] font-semibold text-blue-700 mb-0.5">{label}</label>
                              <div className="relative">
                                <input
                                  type="number"
                                  min="0"
                                  step="0.01"
                                  value={formData.tarifasAlquiler?.[key] ?? ''}
                                  onChange={e => updateAddTarifa(key, e.target.value)}
                                  className="w-full border border-blue-200 rounded-lg pl-2 pr-6 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 bg-white"
                                  placeholder="0"
                                />
                ...
                {hasAlquileres && (
                  <div className="bg-blue-50 border border-blue-100 rounded-xl p-3">
                    <label className="flex items-center gap-2.5 cursor-pointer select-none">
                      <input
                        type="checkbox"
                        checked={editFormData.esFlotaAlquiler ?? false}
                        onChange={e => setEditFormData({ ...editFormData, esFlotaAlquiler: e.target.checked })}
                        className="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                      />
                      <span className="text-xs font-bold text-blue-800">Vehículo de alquiler</span>
                    </label>
                            ...
                            <div key={key}>
                              ...
                              <div className="relative">
                                <input
                                  ...
                                  min="0"
                                  step="0.01"
                                  value={editFormData.tarifasAlquiler?.[key] ?? ''}
                                  onChange={e => updateEditTarifa(key, e.target.value)}
                                  className="w-full border border-blue-200 rounded-lg pl-2 pr-6 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 bg-white"
                                  placeholder="0"
                                />
                                <span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs text-slate-400">€</span>
