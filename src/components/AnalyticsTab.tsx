...
import { OrdenTrabajo, Cliente, OTEstado, Reserva, Vehiculo } from '../types';
import {
...
export default function AnalyticsTab({ ordenesTrabajo, clientes, reservas, vehiculos }: AnalyticsTabProps) {
  ...
  // 10. Rentabilidad de la flota de alquiler (acumulado, por vehículo):
  //     ingresos por alquiler vs. inversión en taller.
  const flotaStats = useMemo(() => {
    return vehiculos
      .filter(v => v.esFlotaAlquiler)
      .map(v => {
        const reservasVeh = reservas.filter(r => r.vehiculoId === v.id && r.estado !== 'cancelada');
        const ingresos = reservasVeh.reduce((s, r) => s + r.totalCobrado, 0);
        const inversionTaller = ordenesTrabajo
          .filter(ot => ot.vehiculoId === v.id && ot.estado !== 'cancelado')
          .reduce((s, ot) => s + ot.total, 0);
        return {
          id: v.id,
          nombre: `${v.marca} ${v.modelo}`,
          matricula: v.matricula,
          alquileres: reservasVeh.length,
          ingresos,
          inversionTaller,
          balance: ingresos - inversionTaller,
        };
      })
      .sort((a, b) => b.balance - a.balance);
  }, [vehiculos, reservas, ordenesTrabajo]);

  const flotaTotales = useMemo(() =>
    flotaStats.reduce(
      (acc, f) => ({
        ingresos: acc.ingresos + f.ingresos,
        inversion: acc.inversion + f.inversionTaller,
        balance: acc.balance + f.balance,
      }),
      { ingresos: 0, inversion: 0, balance: 0 }
    ),
    [flotaStats]
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-lg font-black text-slate-800">Rentabilidad Taller</h2>
          <p className="text-xs text-slate-400 mt-0.5">Indicadores clave del taller</p>
        </div>
      ...
      {/* Rentabilidad de la flota de alquiler */}
      {flotaStats.length > 0 && (
        <div className="space-y-4 pt-2">
          <div>
            <h2 className="text-lg font-black text-slate-800 flex items-center gap-2">
              <Car className="w-5 h-5 text-violet-500" /> Rentabilidad Alquileres
            </h2>
            <p className="text-xs text-slate-400 mt-0.5">Ingresos por alquiler vs. inversión en taller, por vehículo (datos acumulados)</p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 overflow-x-auto">
            <h3 className="text-sm font-bold text-slate-700 mb-4 flex items-center gap-2">
              <BarChart2 className="w-4 h-4 text-violet-500" /> Detalle por vehículo
            </h3>
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="text-slate-400 font-semibold uppercase tracking-wider border-b border-slate-100">
                  ...
                  <th className="py-2 px-2 text-center">Alquileres</th>
                  <th className="py-2 px-2 text-right">Ingresos</th>
                  <th className="py-2 px-2 text-right">Inversión taller</th>
