...
export default function CrmTab({
  ...
  const handleExportCsv = () => {
    ...
    downloadCsv(`${slugify(getEmpresaConfig().nombre)}_clientes_${new Date().toISOString().slice(0, 10)}.csv`, [headers, ...rows]);
  };
          ...
          {selectedCliente ? (
            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-6" id={`crm-detail-${selectedCliente.id}`}>
              ...
              <div className="space-y-3">
                ...
                {selectedCliente.esClienteAlquiler && (
                  <div className="flex items-center gap-2 text-xs bg-blue-50 border border-blue-100 rounded-lg px-3 py-2">
                    ...
                    <span className="font-semibold text-blue-700">Cliente de vehículo de alquiler</span>
                  </div>
                )}
              </div>
              ...
              {/* Historic Alquileres / Rents list */}
            ...
            <div className="bg-white p-12 rounded-2xl border border-slate-100 shadow-sm text-center flex flex-col items-center justify-center space-y-3 text-slate-400 h-full min-h-[400px]">
              ...
              <p className="text-xs max-w-xs">
                Seleccione un cliente del directorio para visualizar sus datos de contacto completos, historial de llamadas o visitas anotadas, y registro de contratos de alquiler.
              </p>
            </div>
          )}
        </div>
                ...
                {hasAlquileres && (
                  <div>
                    <label className="flex items-center gap-2.5 cursor-pointer select-none">
                      <input
                        type="checkbox"
                        checked={formData.esClienteAlquiler ?? false}
                        onChange={e => setFormData({ ...formData, esClienteAlquiler: e.target.checked, vehiculosAsociados: e.target.checked ? [] : formData.vehiculosAsociados })}
                        className="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                      />
                      <span className="text-xs font-bold text-slate-700">Cliente de vehículo de alquiler</span>
                    </label>
                    <p className="text-[11px] text-slate-400 mt-1 ml-6">Los clientes de alquiler no tienen vehículo propio asignado.</p>
                  </div>
                )}

                {/* Vehículos asociados */}
                {vehiculos.length > 0 && !formData.esClienteAlquiler && (
                  <div>
                    ...
                    <div className="space-y-1.5 max-h-40 overflow-y-auto pr-1">
                      {vehiculos
                        .filter(v => !vehiculosAsignadosGlobal.has(v.id) && !v.esFlotaAlquiler)
                      ...
                      {vehiculos.filter(v => !vehiculosAsignadosGlobal.has(v.id) && !v.esFlotaAlquiler && `${v.marca} ${v.modelo} ${v.matricula}`.toLowerCase().includes(vehiculoSearch.toLowerCase())).length === 0 && (
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 overflow-y-auto">
            ...
            >
              ...
              <form onSubmit={handleEditSubmit} className="p-6 space-y-4 font-sans overflow-y-auto flex-1">
                ...
                {hasAlquileres && (
                  <div>
                    <label className="flex items-center gap-2.5 cursor-pointer select-none">
                      <input
                        type="checkbox"
                        checked={editFormData.esClienteAlquiler ?? false}
                        onChange={e => setEditFormData({ ...editFormData, esClienteAlquiler: e.target.checked, vehiculosAsociados: e.target.checked ? [] : (editFormData.vehiculosAsociados ?? []) })}
                        className="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                      />
                      <span className="text-xs font-bold text-slate-700">Cliente de vehículo de alquiler</span>
                    </label>
                ...
                {vehiculos.length > 0 && !editFormData.esClienteAlquiler && (
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Vehículos asociados</label>
                    <div className="relative mb-2">
                      <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 pointer-events-none" />
                    ...
                    <div className="space-y-1.5 max-h-40 overflow-y-auto pr-1">
                      {vehiculos
                        .filter(v => {
                          ...
                          return !asignadoAOtro && (!v.esFlotaAlquiler || propioDelCliente);
                        })
                      ...
                      {vehiculos.filter(v => {
                        ...
                        return !(vehiculosAsignadosGlobal.has(v.id) && !propio) && (!v.esFlotaAlquiler || propio) && `${v.marca} ${v.modelo} ${v.matricula}`.toLowerCase().includes(vehiculoSearchEdit.toLowerCase());
                      }).length === 0 && (
